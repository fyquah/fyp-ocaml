...
In order to override inlining decisions, we have to add some id-ing to call
sites to uniquely identify them.

There are a few places where this id-ing system is lacking, notably:

- [Inline_and_simplify.simplify_partial_application], where we simply bump
  the id, and rely on the fact that nothing is never inlined to prevent
  missed tree branches. Ideally we want some manner of identifying
  these dynamically created fun-vars used in over applications (or just
  ignore them consistently throughout)
- Specialized function is not handled correctly yet.
- [Inlining_decision.ml#390] It should be save to keep [Above_threshold]
  for inlining_definitely_terminates.
