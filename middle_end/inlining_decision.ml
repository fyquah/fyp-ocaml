...
let inline env r ~apply_id ~kind ~call_site ~lhs_of_application
  ...
  let try_inlining =
    ...
    else if not (E.inlining_allowed env function_decl.real_closure_origin) then
  ...
  let always_inline =
    let true_if_inline x =
      match x with
      | Data_collector.Action.Inline -> true
      | _ -> false
    in
    match found with
    ...
    | Some Apply
    | None ->
      match custom_decision with
      | None -> always_inline
      | Some d -> true_if_inline d
  in
  let try_inlining =
    match found with
    ...
    | None ->
      ...
      | Some Data_collector.Action.Inline ->
        Try_it
      | Some Apply ->
        Don't_try_it S.Not_inlined.Override
      | Some Data_collector.Action.Specialise
      | None ->
        try_inlining
  in
  ...
  | Try_it ->
    ...
    let keep_inlined_version decision =
      ...
      in
      let env = E.inside_inlined_function env function_decl.real_closure_origin in
      let env =
