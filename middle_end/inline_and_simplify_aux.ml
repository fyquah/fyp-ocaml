...
module Env = struct
  ...
  let create ~never_inline ~backend ~round ~current_function:current_closure ~overrides =
    { backend;
      ...
      inlining_counts = Real_closure_origin.Map.empty;
  ...
  let inlining_allowed t id =
    let inlining_count =
      try
        Real_closure_origin.Map.find id t.inlining_counts
      with Not_found ->
  ...
  let inside_inlined_function t applied  =
    ...
    in
    let inlining_counts =
      Real_closure_origin.Map.add applied (inlining_count - 1) t.inlining_counts
    in
  ...
  let enter_closure t ~closure_id ~closure_origin ~specialised_for ~set_of_closures_id
