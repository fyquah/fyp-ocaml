...
let tupled_function_call_stub original_params unboxed_version ~closure_bound_var
  ...
  let call : Flambda.t =
    Apply ({
        ...
        apply_id = Apply_id.create `Plain_apply;
      })
  in
  let _, body =
  ...
  Flambda.create_function_declaration ~params:[tuple_param]
    ...
    ~real_closure_origin:(Real_closure_origin.create (Closure_id.wrap closure_bound_var))
...
and close_functions t external_env function_declarations : Flambda.named =
  ...
  let close_one_function map decl =
    ...
    let real_closure_origin =
      Real_closure_origin.create (Closure_id.wrap unboxed_version)
    in
    let fun_decl =
